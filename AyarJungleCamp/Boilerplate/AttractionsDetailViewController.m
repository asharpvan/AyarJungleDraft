//
//  AttractionsDetailViewController.m
//  Boilerplate
//
...
@interface AttractionsDetailViewController () {
    
    AttractionDataModel *attractionData;
...
@implementation AttractionsDetailViewController
...
-(instancetype) initWithAttraction:(AttractionDataModel *) attractionRecieved showAsParallaxView:(BOOL) isParallax {
    ...
    if(self) {
        ...
        attractionData = attractionRecieved;
        isSingleImageParallax = isParallax;
        NSLog(@"attractionData : %@",attractionData);
        [self createSubview];
    }
    return self;
}
...
-(void) createSubview {
    ...
    self.topView = [[TopBarView alloc]initWithTitle:[attractionData attractionName] andSize:CGSizeMake(self.view.bounds.size.width, 64)];
...
- (UIScrollView *)nonParallaxScrollHeaderView {
    ...
    if(!_nonParallaxScrollHeaderView) {
        ...
        for(int i=0; i<3; i++){
            ...
            [parallaxImageView setImage:[UIImage imageNamed:[attractionData attractionImageName]]];
...
#pragma mark <Add subview here for Content View>
        ...
        if(attractionData == nil) {
            ...
            [addr1Label setText:[attractionData attractionCity]];
            ...
            [addr2Label setText:[NSString stringWithFormat:@"%@ - %@",[attractionData attractionState],[attractionData attractionPin]]];
            ...
            [hotelSynopsisLabel setText:[NSString stringWithFormat:@"%f - %f", [attractionData attractionCoordinates].latitude,[attractionData attractionCoordinates].longitude]];
            ...
            [hotelHistory setText:[attractionData attractionContact]];
            ...
            NSString *distanceAsComparedTo = nil;
            if([attractionData isAttractionInNainital]) {
                distanceAsComparedTo = @"Ayar Jungle Camp";
            }else {
                distanceAsComparedTo = @"Nainital";
            }
            [foodRating setText:[NSString stringWithFormat:@"Distance From %@ : %@ kms",distanceAsComparedTo,[attractionData attractionDistance]]];
